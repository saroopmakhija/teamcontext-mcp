...
def get_auth_token() -> str:
    ...
    if not auth_header:
        print(f"❌ No Authorization header found. Available headers: {list(headers.keys())}")
...
def edit_project(
    ...
    try:
        if not name and not description:
            return "❌ Must provide at least name or description to update"
...
def add_context(
    ...
    Returns:
        Success message with context ID
    """
    try:
        auth_token = get_auth_token()
        ...
        return f"✅ Context saved successfully!\nContext ID: {result['context_id']}\nProject: {project_id}"
    except Exception as e:
